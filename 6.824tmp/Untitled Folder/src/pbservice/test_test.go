...
func TestFailPut(t *testing.T) {
	...
	if v2.Primary != s1.me || v2.Backup != s3.me {
		t.Fatal("wrong primary or backup", v2.Primary, v2.Backup)
	}
...
func TestRepeatedCrashUnreliable(t *testing.T) {
	...
	go func() {
		...
		for atomic.LoadInt32(&done) == 0 {
			...
			dprint("%s is backup before %s go to die\n", sa[k].me, sa[i].me)
			for key, value := range sa[k].pairs {
				dprint("%s - %s\n", key, value)
			}
			sa[i].kill()
	...
	ff := func(i int, ch chan int) {
		ret := -1
		defer func() { ch <- ret }()
		ck := MakeClerk(vshost, strconv.Itoa(i))
		n := 0
		for atomic.LoadInt32(&done) == 0 {
			v := "x " + strconv.Itoa(i) + " " + strconv.Itoa(n) + " y"
			ck.Append("0", v)
